...
0
*/
/*  For this question this code works, but this code is wrong, Real code will not give 100*/
/*
 * Real code
 * 
import java.util.*;
class program1{
    public static void main (String[] args) {
        ...
        for(int i=0;i<n;i++) down[i]=sc.nextInt();
        sc.close();
        System.out.println(find(n,up,down));
    }
    public static int find(int n,int u[],int d[]){
        int ans=Integer.MAX_VALUE;
        for(int i=0;i<n;i++){
            if(u[i]==d[i]){
                continue;
            }
            if(check(u[i],i,u,d,n)){
                ans=Math.min(ans,u[i]);
            }
            if(check(d[i],i,u,d,n)){
                ans=Math.min(ans,d[i]);
            }
        }
        return ans==Integer.MAX_VALUE?0:ans;
    }
    public static boolean check(int el,int i,int u[],int d[],int n){
        for(int j=0;j<n;j++){
            if(i==j) continue;
            if(el==u[j] && el==d[j]) return false;
        }
        return true;
    }
}
...
class program1{
    public static void main (String[] args) {
        ...
        int ans=Integer.MAX_VALUE;
        for(int i=0;i<n;i++){
            if(u.get(i)==d.get(i)) continue;
            if(!d.contains(u.get(i))){
                ans=Math.min(ans,u.get(i));
            }
            if(!u.contains(d.get(i))){
                ans=Math.min(ans,d.get(i));
            }
        }
